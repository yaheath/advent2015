...
fn part1(input: &[u64]) -> u64 {
    ...
    for s in input.iter().copied().powerset().filter(|s| !s.is_empty()) {
        ...
        for s1 in all.difference(&set).copied().powerset().filter(|s| !s.is_empty()) {
    ...
    for s in input.iter().copied().powerset().filter(|s| !s.is_empty()) {
        ...
        for s1 in all.difference(&set).copied().powerset().filter(|s| !s.is_empty()) {
            if s1.iter().sum::<u64>() == w {
                ...
                for s2 in set.difference(&set1).copied().powerset().filter(|s| !s.is_empty()) {
